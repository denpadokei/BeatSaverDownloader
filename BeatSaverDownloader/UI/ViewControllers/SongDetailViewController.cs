...
using BeatSaverBeatmapDifficulty = BeatSaverSharp.Models.BeatmapDifficulty;
...
namespace BeatSaverDownloader.UI.ViewControllers
{
    public class SongDetailViewController : BeatSaberMarkupLanguage.ViewControllers.BSMLResourceViewController, INotifiableHost
    {
        ...
        private BeatSaverBeatmapDifficulty[] _currentDifficulties;
        ...
        internal async void Initialize(StrongBox<Beatmap> song, Sprite cover)
        {
            ...
            SelectedCharacteristic(_currentSong.LatestVersion.Difficulties[0].Characteristic);
        ...
        internal void SetupDetailView()
        {
            ...
            _characteristicSegmentedControl = CreateIconSegmentedControl(_characteristicSegmentedControllerClone.transform as RectTransform, new Vector2(0, 0), new Vector2(0, 0),
                delegate (int value) { SelectedCharacteristic(_currentSong.LatestVersion.Difficulties[value].Characteristic); });
        ...
        {
            var diffs = new List<BeatSaverSharp.Models.BeatmapDifficulty>();
            List<string> diffNames = new List<string>(_currentDifficulties.Select(x => x.Difficulty.ToString()).OrderBy(x => DiffOrder(x))).ToList();
            foreach (var diff in diffNames)
            {
                if (_currentDifficulties.Select(x => x.Difficulty.ToString()).Contains(diff))
                {
                    var diffTmp = _currentDifficulties.FirstOrDefault(x => x.Characteristic.ToString() == diff);
                    if (diffTmp != null) diffs.Add(diffTmp);
                }
            }
        ...
        private void SetupCharacteristicDisplay()
        {
            ...
            foreach (var c in _currentSong.LatestVersion.Difficulties)
            {
                BeatmapCharacteristicSO characteristic = SongCore.Loader.beatmapCharacteristicCollection.GetBeatmapCharacteristicBySerializedName(c.Difficulty.ToString());
                if (characteristic.characteristicNameLocalizationKey == "Missing Characteristic")
                {
                    characteristics.Add(new IconSegmentedControl.DataItem(characteristic.icon, $"Missing Characteristic: {c.Difficulty}"));
                }
                else
